...
#define PWM_PERIOD 20000 // 50 Hz with 16 MHz clock and /16 prescaler
...
void ServoBoard::begin()
{
  ...
  TCCR1A = _BV(COM1A1); // No output on pins
  TCCR1B = _BV(WGM13); // Phase/frequency correct PWM to ICR1
  OCR1A = 2; // Pulse duration
  ICR1 = PWM_PERIOD;
...
{  
  ...
  for (byte i = 0; i < NUMBER_OF_SERVO_CHANNELS / 2; ++i) {
    word first = servoTo12bits(channelValues[NUMBER_OF_SERVO_CHANNELS - 1 - 2*i]);
    word second = servoTo12bits(channelValues[NUMBER_OF_SERVO_CHANNELS - 2 - 2*i]);
