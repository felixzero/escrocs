...
static QueueHandle_t closest_obstacle_queue, proximity_queue;
...
void request_cb(uint8_t num, void *arg) {
    i2cSlaveWrite(num, tx_buffer, data_len, 1000);
    data_len = 0;
}
...
void receive_cb(uint8_t num, uint8_t *data, size_t len, bool stop, void *arg) {
    ...
    case I2C_REG_BOOL_OBSTACLE:
        bool proximity = 1;
        xQueuePeek(proximity_queue, &proximity, 0);
        tx_buffer[0] = (uint8_t) proximity + 4;
        data_len = 1;
        break;
    case I2C_REG_MM_OBSTACLE:
        uint16_t dist = 0;
        xQueuePeek(closest_obstacle_queue, &dist, 0);
        dist = (dist == 0) ? 10 : dist;
...
void i2c_slave_task(void *pvParameters) {
    ...
    closest_obstacle_queue = (QueueHandle_t) xQueueCreate(1, sizeof(uint16_t));
    proximity_queue = (QueueHandle_t) xQueueCreate(1, sizeof(bool));
    ...
    while (true) {
        ...
        dist = closest_obstacle_dist();
        xQueueOverwrite(closest_obstacle_queue, &dist);
        proximity = has_obstacle();
        xQueueOverwrite(proximity_queue, &proximity);
